...
# Uso: ./scripts/deploy.sh [dev|prod] [start|stop|restart|logs|status]
...
show_status() {
    ...
    # PostgreSQL
    if docker exec psycho-postgres pg_isready -U psycho_manager -d psycho_manager > /dev/null 2>&1; then
...
backup_database() {
    ...
    if docker exec psycho-postgres pg_dump -U psycho_manager psycho_manager > $BACKUP_FILE; then
...
restore_database() {
    ...
    if docker exec -i psycho-postgres psql -U psycho_manager -d psycho_manager < $BACKUP_FILE; then
